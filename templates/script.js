...
function computeCellNumbers() {
    ...
    for (let r = 0; r < n; r++) {
        for (let c = 0; c < n; c++) {
            ...
            const startsAcross = (c === 0 || state.black[r][c-1]) && (c+1 < n && !state.black[r][c+1]);
            const startsDown   = (r === 0 || state.black[r-1][c]) && (r+1 < n && !state.black[r+1][c]);
...
function onCellClick(r, c) {
    ...
    const clickedSame = state.activeCell && state.activeCell[0] === r && state.activeCell[1] === c;
    ...
    if (clickedSame) {
        ...
        nextDir = here[state.dir] !== null ? state.dir : (here.across !== null ? 'across' : 'down');
    }
...
function selectSlot(idx, moveCellToStart) {
    ...
    if (moveCellToStart || !state.activeCell || !cellInSlot(state.activeCell, slot)) {
...
function refreshHighlights() {
    // clear
    document.querySelectorAll('.cell').forEach(el => el.classList.remove('word', 'cur'));
    document.querySelectorAll('.clue').forEach(el => el.classList.remove('sel'));
    ...
    if (state.activeCell) {
        ...
        if (el) { el.classList.remove('word'); el.classList.add('cur'); }
    }

    // sync the one highlighted clue
    const clueRow = document.querySelector(`.clue[data-idx="${state.activeSlot}"]`);
...
function advance(step) {
    ...
    let i = cells.findIndex(([r, c]) => r === state.activeCell[0] && c === state.activeCell[1]);
    i += step;
    if (i < 0) i = 0;
...
async function submitGuess(slotIdx) {
    ...
    const guess = slot.cells.map(([r, c]) => state.entries[r + ',' + c] || '').join('');
...
function playFeedback(feedback, correct) {
    ...
    setTimeout(() => { fx.innerHTML = ''; }, 1100);
}

function feedbackStyle(feedback, correct) {
    const f = (feedback || '').toLowerCase();
    if (f === 'hint') {
        return { label: 'Hint', bg: '#00e7ff', fg: '#062028',
                 pillAnim: 'pulsePill .9s ease-out', particles: (fx)=>sparks(fx,['#00e7ff','#5ad4ff']) };
    }
    if (correct || f === 'correct!') {
        return { label: 'Correct!', bg: '#2bff9a', fg: '#062028',
                 pillAnim: 'burstPill 1s ease-out', particles: confetti };
    }
    if (f.includes('so close')) {
        return { label: 'Ooh so close', bg: '#ff4d7d', fg: '#fff',
                 pillAnim: 'pulsePill .9s ease-out', particles: (fx)=>poppers(fx,'#ff4d7d') };
    }
    if (f === 'warm' || f.includes('warm')) {
        return { label: 'Warm', bg: '#ff9f2e', fg: '#062028',
                 pillAnim: 'pulsePill .9s ease-out', particles: (fx)=>sparks(fx,['#ff9f2e','#ffd84d']) };
    }
    if (f.includes('maybe')) {
        return { label: 'Maybe', bg: '#ffd84d', fg: '#062028',
                 pillAnim: 'pulsePill .9s ease-out', particles: (fx)=>sparks(fx,['#ffd84d','#5ad4ff']) };
    }
    if (f.includes('cool')) {
        return { label: 'Cool', bg: '#5ad4ff', fg: '#062028',
                 pillAnim: 'pulsePill .9s ease-out', particles: (fx)=>snow(fx,4) };
    }
    if (f.includes('freezing')) {
        return { label: 'Freezing', bg: '#12324a', fg: '#7fb9d6',
                 pillAnim: 'pulsePill .9s ease-out', particles: (fx)=>snow(fx,9) };
    }
    // not even close
    return { label: 'Not even close', bg: '#12324a', fg: '#7fb9d6',
             pillAnim: 'shakePill .5s ease-out', particles: null };
}
...
function playWin() {
    ...
    const tierName = { bitsize: 'Bitsize shallows', mega: 'Mega reef', giga: 'Giga abyss' }[state.tier] || 'puzzle';
    document.getElementById('win-sub').textContent = 'You cleared the ' + tierName;
...
async function useHint() {
    ...
    for (let i = 0; i < slot.cells.length; i++) {
        ...
        if (!state.entries[r + ',' + c]) { pos = i; break; }
    }
    ...
    try {
        ...
        if (data.letter) {
            ...
            state.hintsUsed++;
            playFeedback('hint', false);   // cyan hint shimmer only — no warmth
            advanceToNextEmpty();
            // if the hint filled the LAST empty cell, check the word silently
            // (mark solved if correct, but never show a warm/cold message from a hint)
...
function init() {
    ...
    if (btnNew) btnNew.addEventListener('click', () => { stopTimer(); showScreen('landing'); });
    ...
    if (btnHome) btnHome.addEventListener('click', () => { stopTimer(); showScreen('landing'); });
